...
DEFAULT_CONFIG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'modules', 'config.yaml')
...
def main():
    ...
    # Check for root privileges (warn but don't exit on Windows)
    if not check_privileges():
        print("[WARNING] Not running as Administrator. Some features may not work.")
        if sys.platform == "win32":
            # On Windows, allow continuing without admin (user can see warnings)
            pass
        else:
...
def run_firewall(config, logger):
    ...
    global threads
    try:
        # Initialize the management client with full config
        logger.info("Initializing management client...")
        management_client = ManagementClient(config)
        ...
        if config.get('dpi', {}).get('enabled', False):
            ...
            try:
                ...
                logger.warning(f"DPI module failed to initialize: {e} - continuing without it")
        ...
        if config.get('edr', {}).get('enabled', False):
            ...
            try:
                ...
                logger.warning(f"EDR module failed to initialize: {e} - continuing without it")
        ...
        if config.get('hips', {}).get('enabled', False):
            ...
            try:
                ...
                logger.warning(f"HIPS module failed to initialize: {e} - continuing without it")
        ...
        for thread in threads:
            try:
                ...
                logger.error(f"Failed to start {thread.__class__.__name__}: {e}")
